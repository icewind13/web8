...
const symbols = ['AAPL', 'GOOG', 'SBUX', 'NKE', 'INTC', 'FB', 'TWTR', 'MCD'];
...
function batch(fields) {
    const url = `` +
        ...
        `&filter=${fields.join()}` +
        `&token=${token}`;
    return from(fetch(url).then(res => res.json()).catch(err => alert('Ошибка: ' + err)));
}
...
(function () {
    ...
    update
        ...
        .subscribe(val => {
            info.innerHTML = `С момента последнего обновления прошло: ${(val * 0.1).toFixed(1)}секунд`;
        });
